...
function laukums(m1, m2, m3) {
    const pusper = perimetrs(m1, m2, m3);
...
function rezultats() {
    ...
    console.log(m1, m2, m3);
    if (!nolasa()) {
        t = " Nekorekti ievaddati, trijstura malu vertibam ir jabut lielakam par 0 ";
    } else {
        if (irTrijsturis(m1, m2, m3) == false) {
            t = " Trijsturis neeksiste, jo jebkuru 2 malu garumu sumai ir jabut lielakai par treso malu! ";
        } else {
            t = " Trijsturis ar malu garumiem " + m1 + "," + m2 + " un" + m3 + " eksiste."
            if (m1 == m2 && m2 == m3) {
                t += " Tas ir vienadmalu trijsturis."
            } else {
